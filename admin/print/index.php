...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->SetFont('', 'B', 10);
        // $this->Cell(0, 5, 'JONATHAN LIBRARY ', 0, 1, 'C');
        // $this->Cell(0, 5, 'LIST OF MATERIALS ', 0, 1, 'C');
...
// $studentssss = Student::find_by_query("SELECT * FROM student WHERE current_class_id = $class->id");
...
foreach ($students as $student) {
    ...
    $ca_tests = Ca_test::find_by_query("SELECT * FROM $db_table WHERE roll_no=$student->id");
    ...
    endforeach;
    array_push($score_array, $total_sub_scores);
}

// array_search();
array_multisort($score_array, SORT_DESC);
foreach ($students as $student) {
    ...
    $position = array_search($total_sub_scores, $score_array) + 1;

    $pdf->Ln(5);

    $pdf->Cell(18, 5, 'Total Mark:', 0, 0, 'L', false);
    $pdf->Cell(25, 5, $total_sub_scores, 'B', 0, 'C', false);

    $pdf->Cell(12, 5, 'Out Of:', 0, 0, 'L', false);
    $pdf->Cell(35, 5, $out_of, 'B', 1, 'C', false);

    $pdf->Ln(3);
    $pdf->Cell(23, 5, 'Class Position:', 0, 0, 'L', false);
    $pdf->Cell(20, 5, $position, 'B', 0, 'C', false);

    $pdf->Cell(27, 5, 'Numbers in Class:', 0, 0, 'L', false);
    $pdf->Cell(20, 5, count($students), 'B', 1, 'C', false);
    ...
    $filename = "../result_sheets/" . $student->id . ".pdf";
    $pdf->Output($filename, 'F');
}
header("Location: ../email/mail_dev.php?class_id=$class_id");
